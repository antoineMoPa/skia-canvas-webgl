...
const {Image} = require('@napi-rs/canvas/js-binding')
...
class NodeCanvasElement {
  ...
  set height(value) {
    ...
    if(this.__gl__) {
      const ext = this.__gl__.getExtension('STACKGL_resize_drawingbuffer');
      ext.resize(this.canvas.width, value);
    }
 ...
 getContext(type, options) {
    ...
    if(type === 'webgl' || type === 'webgl2') {
      ...
      this[_ctx] = this.canvas.getContext('2d', options);
      ...
      ctx.texImage2D = function (...args) {
        ...
        if(pixels instanceof Image) {
          const canvas = new NodeCanvasElement(pixels.width, pixels.height);
  ...
  createJPEGStream(...args) {
    ...
    return this.canvas.createJPEGStream(...args);
  }
  ...
  createPDFStream(...args) {
    ...
    return this.canvas.createPDFStream(...args);
  }
